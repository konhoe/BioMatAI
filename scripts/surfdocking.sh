...
if [[ -z "$PROTEIN_CHAIN" ]]; then PROTEIN_CHAIN="A"; fi
if [[ -z "$SURFACE_CHAIN" ]]; then SURFACE_CHAIN="Z"; fi
...
if [[ -f "$USER_SURFACE_VECTORS" ]]; then
    ...
    echo "[INFO] surface_vectors 자동 생성 시도"
...
if [[ -f "$FRAG3_SRC" ]]; then
    ...
    echo "[WARN] 3mer fragment 없음: $FRAG3_SRC"
fi
...
if [[ -f "$FRAG9_SRC" ]]; then
    ...
    echo "[WARN] 9mer fragment 없음: $FRAG9_SRC"
fi
